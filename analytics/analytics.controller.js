...
exports.timesheetsFilled = async function (req, res) {
  ...
  const onTimePercentage = (
    (filledOnOrBeforeEndDate / totalTimesheets) *
    100
  ).toFixed(2);
  const notOnTimePercentage = (
    (filledAfterEndDate / totalTimesheets) *
    100
  ).toFixed(2);
...
exports.getDataforPlotting = async function (req, res) {
  try {
    ...
    for (const timesheet of timesheets) {
      ...
      const isBillable = resourceMap?.isClientBillable?.from ? true : false;
      ...
      for (let i = 0; i < timesheet.totalHours.length; i++) {
        ...
        if (!from) {
          //if from date not given then hours are non-billable
          nonBillableHours += timesheet.totalHours[i];
        } else if (from && !till) {
          // if from data is given then hours are billable
